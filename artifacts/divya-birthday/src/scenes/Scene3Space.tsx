...
export function Scene3Space({ onComplete, playSound }: { onComplete: () => void, playSound: (s: any) => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Planets drifting by */}
      <motion.div 
        className="absolute w-32 h-32 rounded-full bg-gradient-to-br from-orange-400 to-red-600 blur-[2px]"
        initial={{ x: '110vw', y: '20vh' }}
        animate={{ x: '-20vw', y: '40vh' }}
        transition={{ duration: 5, ease: "linear" }}
      />
      <motion.div 
        className="absolute w-64 h-64 rounded-full bg-gradient-to-tr from-purple-800 to-indigo-500 blur-[4px] opacity-60"
      ...
      {/* HUD */}
      <div className="absolute bottom-8 left-8 glass-panel p-4 text-cyan-300 font-mono z-20">
        <div>Speed: {Math.floor(speed * 450)} km/s</div>
        <div>Sector: Deep Space</div>
        <div className="mt-2">
          Love Energy:
          <div className="w-48 h-4 bg-black/50 border border-cyan-500/50 mt-1 relative overflow-hidden">
        ...
        {locked && (
          <>
            <motion.div 
              ...
              className="absolute top-20 left-1/2 -translate-x-1/2 text-4xl md:text-6xl text-pink-400 font-bold text-glow-pink z-40 text-center w-full"
            >
              DESTINATION LOCKED
            </motion.div>
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 15, rotate: 180 }}
              transition={{ duration: 3, ease: "easeIn" }}
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 rounded-full border-[20px] border-pink-500/80 z-20 shadow-[0_0_100px_#ff69b4]"
              style={{
